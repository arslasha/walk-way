...
9. **Product Strategy & PRD Compliance:** Before starting any new task or feature, you MUST check the `docs/prd/` and `docs/` directories for relevant Product Requirements Documents (PRDs) or strategy files (e.g., `LLM_STRATEGY.md`) generated by the Product Manager agent. You must read and strictly follow the functional requirements, edge cases, and acceptance criteria outlined in these documents during your execution.